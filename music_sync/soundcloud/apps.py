...
from music_sync.core.download import ArchiveParser, Downloader
...
class SoundCloudApp(App):
    ...
    class ConfigSchema(App.ConfigSchema):
        ...
        dir = ContextAware(
            PathField, required=True, context_key="base_path", dump_default="SoundCloud"
        )
    ...
    def sync(self):
        if self.config.username == "":
            raise ValueError(
                "You should configure an username for SoundCloud or "
                + f"disable SoundCloud sync in {get_config_path(writable=True)}"
            )
        ...
        if self.config.download_archive:
            try:
                archived_ids = [
                    id
                    for provider, id in ArchiveParser(
                        self.config.download_archive
                    ).read_rows()
        ...
        urls = [
            track["permalink_url"]
            for track in tracks
            if f"{track['id']}" not in archived_ids
        ]
