...
def _get_config_path(*resource, config_file='config.yaml', writable=False):
    """
    If writable is false return the path of the first readable config file following xdg specs.\n
    If writable is true return the path of the first readable and writable config file following xdg specs.\n
    If no matching files are found return the path were the config file should be created.
    """
    for config_dir in xdg.BaseDirectory.load_config_paths(*resource):
        ...
        if os.path.isfile(config_file_path) and os.access(config_file_path, os.R_OK):
            if not writable or os.access(config_file_path, os.W_OK):
...
def read_config(*resource, config_file='config.yaml'):
    ...
    with open(config_file_path, "r") as stream:
        ...
        logging.getLogger(__name__).debug(
            f'Read config in {config_file_path}')
        return config
