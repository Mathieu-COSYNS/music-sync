...
class App(ABC):
    class ConfigSchema(Schema):
        ...
        def to_obj(self, data, **kwargs):
            name = self.__class__.__qualname__.replace(".", "").replace("Schema", "")
    ...
    def __init__(self, config) -> None:
        assert re.match("^[a-z0-9_]+$", self.name) is not None, (
            f"{self.__class__.__name__} name is {self.name} but should only contains "
            + "lower case a-z0-9 characters or underscore."
        )
    ...
    def name(self) -> str:
        raise AssertionError(
            "App {self.__class__.__name__} does not have a name attribute."
        )
